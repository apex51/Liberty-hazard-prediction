...
iter_round = 80 # num of bagging rounds
...
for l1_ratio in [0.5, 0.9]:
    ...
    for i in range(iter_round):
        ...
        print '##################################################################################'
        # level 1's training
        l1, l2= train_test_split(range(train.shape[0]), train_size=l1_ratio, random_state=i+20)
    ...
    with open('./data/meta_80round_{}.bst'.format(l1_ratio), 'wb') as f:
        pickle.dump(pred, f)

    # preds = pd.DataFrame({"Id": test_ind, "Hazard": pred})
    # preds = preds.set_index('Id')
    # preds.to_csv('meta_20round_output_{}.csv'.format(l1_ratio))
